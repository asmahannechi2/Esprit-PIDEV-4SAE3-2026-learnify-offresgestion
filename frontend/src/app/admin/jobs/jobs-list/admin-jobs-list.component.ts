            ...
            <tr *ngFor="let job of filtered">
              ...
              <td>
                @if (job.salaryMin != null && job.salaryMax != null) {
                  {{ job.salaryMin | number }} – {{ job.salaryMax | number }} TND
                } @else {
                  —
                }
              </td>
              ...
              <td>{{ (job.expiresAt || job.deadline) | date:'mediumDate' }}</td>
              <td>
        ...
        <div class="modal-overlay" (click)="cancelDelete()">
          <div class="modal-content" (click)="$event.stopPropagation()">
            ...
            <div class="modal-body">
              <p>Are you sure you want to delete <strong>{{ toDelete?.titre }}</strong>?</p>
...
export class AdminJobsListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.jobService.getAllJobs().subscribe({
      ...
      error: (err) => {
        console.error('[Admin Jobs] GET /api/jobs failed — vérifie que le proxy envoie vers job-service :8088 (pas user-service :8087).', err);
        this.jobs = [];
        this.filter();
      }
    });
  ...
  filter(): void {
    ...
    if (this.searchTerm) {
      ...
      list = list.filter(j => (j.titre || '').toLowerCase().includes(t) || (j.subject || '').toLowerCase().includes(t));
    }
