...
export class ApiGatewayInterceptor implements HttpInterceptor {
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    // Retry only idempotent reads to avoid duplicate side effects on POST/PUT/PATCH/DELETE.
    const isRetryableMethod = modifiedReq.method === 'GET' || modifiedReq.method === 'HEAD';
    const request$ = next.handle(modifiedReq);

    return (isRetryableMethod ? request$.pipe(retry(1)) : request$).pipe(
  ...
  private handleError(error: HttpErrorResponse): Observable<never> {
    ...
    if (!this.isProduction() && !(error.error instanceof ErrorEvent)) {
      const detail = formatServerErrorBody(error.error);
      if (detail) {
        console.error('[API Gateway] Server message:', detail);
      }
      console.error('[API Gateway] Error body (raw):', error.error);
    }
...
/** Corps d’erreur lisible (ApiErrorResponse { code, message } ou texte brut). */
function formatServerErrorBody(body: unknown): string {
  if (body == null) {
    return '';
  }
  ...
  if (typeof body === 'object') {
    ...
    const msg = o['message'];
    if (msg != null) {
      return (code != null ? `[${code}] ` : '') + String(msg);
    }
  }
  try {
