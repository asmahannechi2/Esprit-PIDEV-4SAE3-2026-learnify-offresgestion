...
export class PaymentComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    // Always resolve real userId from /me before processing payment
    const cached = Number(localStorage.getItem('userId'));
    const userId$ = (cached > 0) ? of(cached) : this.userService.getMe().pipe(
      switchMap(u => {
    ...
    userId$.subscribe({
      next: (userId) => {
        if (!userId) {
          ...
          alert('Could not identify your account. Please log out and log back in.');
          return;
        }
        if (this.isCartCheckout) {
          this.processCartPayment(formData, userId);
        } else {
          this.processSingleCoursePayment(formData, userId);
        }
      },
      error: () => {
        this.isProcessing = false;
        alert('Could not verify your account. Please log out and log back in.');
      }
    });
  }
  ...
  private getUserIdFromToken(): number {
    ...
    try {
      ...
      const raw = payload.userId ?? payload.user_id ?? payload.id ?? payload.sub ?? 0;
      const id = Number(raw);
  ...
  private processCartPayment(formData: any, userId: number): void {
      ...
      next: (response) => {
        ...
        this.enrollmentService.enrollUserInCourses(userId, courseIds).subscribe({
          next: () => {
            ...
            alert('Payment successful! You are now enrolled. Go to Courses to start learning.');
            setTimeout(() => {
  ...
  private processSingleCoursePayment(formData: any, userId: number): void {
