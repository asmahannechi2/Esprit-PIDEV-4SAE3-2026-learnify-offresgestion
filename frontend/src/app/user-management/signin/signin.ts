...
import { jwtRoleFromAccessToken } from '../../guards/auth.guard';
...
export class Signin implements OnInit {
  ...
  private afterAuthSuccess(response: any) {
    ...
    const roleFromJwt = jwtRoleFromAccessToken(token);
    const role = roleFromJwt || roleFromResponse;
    ...
    localStorage.setItem('user', JSON.stringify({ role: role || '', email: email || '' }));
    ...
    this.userService.getMe().subscribe({
      next: (u: unknown) => {
        ...
        const r = jwtRoleFromAccessToken(token) || role || (p.role || '').toUpperCase();
        if (p.id) localStorage.setItem('userId', String(p.id));
        const roleUp = (r || '').toUpperCase();
