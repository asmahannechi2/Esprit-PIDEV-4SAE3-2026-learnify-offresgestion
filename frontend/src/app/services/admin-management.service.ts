...
/** Création candidat par admin (inscription équivalente au signup « candidate »). */
...
export class AdminManagementService {
  ...
  /**
   * Créer un compte candidat (rôle CANDIDATE).
   * Utilise `/api/users/admin/create-candidate` (comme le doublon de create-tutor côté UserController)
   * pour éviter les 404 si seul ce contrôleur est à jour derrière la gateway.
   */
  createCandidate(payload: CreateCandidateRequest): Observable<any> {
    return this.http.post(`${this.usersUrl}/admin/create-candidate`, payload);
  }
