...
export class CourseService {
  ...
  listForUser(userId: number): Observable<CourseDto[]> {
    const params = new HttpParams().set('userId', userId);
    return this.http.get<CourseDto[]>(`${this.base}`, { params });
  }

  completeCourse(courseId: number, body: { userId: number; userName: string; userEmail: string; courseTitle: string }): Observable<any> {
    return this.http.post(`${this.base}/${courseId}/complete`, body, { responseType: 'text' as const });
  }
