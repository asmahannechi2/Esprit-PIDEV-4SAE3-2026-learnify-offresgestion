...
public class ApplicationMatchScoreService {
    ...
    public int computeMatchScore(Application application, JobDTO job) {
        if (job == null) return 10;
        String candidateText = nullToEmpty(application.getMotivation()) + " " + nullToEmpty(application.getCvExtractedText());
        return calculateAdvancedCorrelation(job, candidateText);
    }

    public int computeJobMatchScoreForCvText(String cvText, JobDTO job) {
        return calculateAdvancedCorrelation(job, cvText);
    }

    private int calculateAdvancedCorrelation(JobDTO job, String candidateRawText) {
    ...
    private Map<String, Double> buildJobVector(JobDTO job) {
    ...
    private double checkLanguageLevelCompatibility(JobDTO job, String candidateText) {
    ...
    public boolean containsKeyword(Application application, String keyword) {
        ...
        String text = nullToEmpty(application.getMotivation()) + " " + nullToEmpty(application.getCvExtractedText());
        return text.toLowerCase(Locale.ROOT).contains(k);
    }
