...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleDataIntegrity(DataIntegrityViolationException ex) {
        ...
        log.error("Data integrity violation: {}", root, ex);
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ApiErrorResponse("DATA_INTEGRITY_ERROR", 
                        root.contains("not-null") || root.contains("NULL") 
                        ? "Required field missing: " + root 
                        : "Could not complete the request due to data integrity violation."));
    }
